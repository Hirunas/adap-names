...
/**
 * A name is an immutable sequence of string components separated by a delimiter character.
...
export interface Name extends Cloneable, Printable, Equality {
    ...
    /** Returns a new Name with the component at index i replaced. Expects that c is properly masked */
    setComponent(i: number, c: string): Name;

    /** Returns a new Name with component c inserted at index i. Expects that c is properly masked */
    insert(i: number, c: string): Name;

    /** Returns a new Name with component c appended. Expects that c is properly masked */
